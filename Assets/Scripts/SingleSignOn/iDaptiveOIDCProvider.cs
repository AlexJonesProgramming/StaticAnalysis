...
{
    /// <summary>
    /// Implementation of the OpenID Connect iDaptive Provider
    /// </summary>
    public class iDaptiveOIDCProvider : IOidcProvider
    {
        /// <summary>
        ...
        /// <summary>
        /// Creates a new instance of the iDaptive client
        /// </summary>
        public iDaptiveOIDCProvider()
        {
        ...
        public async Task<string> GetAccessTokenFromCodeAsync(string code, string redirectUri)
        {
            ...
            if (response.Successful)
            {
                ...
                iDaptiveAuthorizationFlowAnswer answer =
                    JsonSerializer.FromJson<iDaptiveAuthorizationFlowAnswer>(response_content);
                if (answer == null)
                {
        ...
        /// <summary>
        /// Gets the access token based on a previously retrieved refresh token
        /// </summary>
        /// <param name="refeshToken">The refresh toke</param>
        /// <param name="redirectUri">The redirect URI which was used during the login</param>
        /// <returns>Returns the access token if it could be retrieved; otherwise it returns an empty string</returns>
        public async Task<string> GetAccessTokenFromRefreshTokenAsync(string refeshToken, string redirectUri)
        {
            if (ClientData == null)
            {
                i5Debug.LogError("No client data supplied for the OpenID Connect Client.\n" +
            ...
            string uri = refreshEndpoint + $"?client_id={ClientData.ClientId}" +
                $"&redirect_uri={redirectUri}&client_secret={ClientData.ClientSecret}&code={refeshToken}&grant_type=authorization_code";
            WebResponse<string> response = await RestConnector.PostAsync(uri, "");
        ...
        /// <summary>
        /// Gets information about the logged in user from the iDaptive provider
        /// </summary>
        ...
        public async Task<IUserInfo> GetUserInfoAsync(string accessToken)
        {
            ...
            {
                iDaptiveUserInfo userInfo = JsonSerializer.FromJson<iDaptiveUserInfo>(webResponse.Content);
                if (userInfo == null)
                {
        ...
        /// <summary>
        /// Checks if the access token is valid by checking it at the iDaptive provider
        /// </summary>
